    ...
    <section className="border-y border-border bg-secondary/20">
      <div className="container py-10 md:py-12">
        <div className="relative overflow-hidden">
          ...
          <div
            className="flex w-max items-center gap-10 md:gap-16 px-8 animate-marquee-ltr motion-reduce:animate-none"
            aria-label="Marques"
          >
            {[...brands, ...brands].map((brand, idx) => (
              <span
                key={`${brand}-${idx}`}
                className="text-base md:text-lg font-bold tracking-wide text-muted-foreground/70 hover:text-foreground transition-colors cursor-default whitespace-nowrap"
              >
                {brand}
              </span>
            ))}
